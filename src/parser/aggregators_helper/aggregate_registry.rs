...
use crate::{
    JsonPrimitive,
    parser::{
        aggregators_helper::{AggregateImpl, AvgImpl, CountImpl, MaxImpl, MinImpl, SumImpl},
...
impl AggregateRegistry {
    ...
    pub fn register<I: AggregateImpl + 'static>(&mut self, impl_: I) {
        self.by_name
            .insert(impl_.name().to_string(), Arc::new(impl_));
    }
    ...
    pub fn infer_type(
        ...
        let imp = self
            .get(&lname)
            .ok_or_else(|| AnalyzerError::FunctionNotFound(fun.name.clone()))?;
...
mod tests {
    ...
    impl SchemaProvider for SP {
        fn schema_of(&self, name: &str) -> Option<SchemaDict> {
            self.by.get(name).cloned()
        }
    }
    ...
    fn registry_infer_type_matches_rules() {
        ...
        assert_eq!(
            r.infer_type(&c_star, &ctx).unwrap(),
        ...
        let sum_i = Function {
            ...
            args: vec![ScalarExpr::Column(
                crate::parser::ast::Column::WithCollection {
                    collection: "t".into(),
                    name: "i".into(),
                },
            )],
        ...
        assert_eq!(
            r.infer_type(&sum_i, &ctx).unwrap(),
            (JsonPrimitive::Int, true)
        );
        ...
        assert_eq!(
            r.infer_type(&avg_f, &ctx).unwrap(),
            (JsonPrimitive::Float, true)
        );
