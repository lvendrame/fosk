...
impl TypeInference {
    pub fn infer_scalar(
        ...
        match expr {
            ...
            ScalarExpr::WildCard | ScalarExpr::WildCardWithCollection(_) => Err(
                AnalyzerError::Other("wildcards should be expanded before type inference".into()),
            ),
            ScalarExpr::Parameter => Err(AnalyzerError::Other(
                "Parameter should be expanded before type inference".into(),
            )),
            ScalarExpr::Args(_) => Err(AnalyzerError::Other(
                "Args should be expanded before type inference".into(),
            )),
        }
    }
    ...
    fn infer_function_type(
        ...
        match (lname.as_str(), arg_types.as_slice()) {
            ...
            | ("trim", [(JsonPrimitive::String, nullable)]) => {
                Ok((JsonPrimitive::String, *nullable))
            }
...
mod tests {
    use crate::{
        ...
        parser::{aggregators_helper::AggregateRegistry, ast::Column},
    };
    ...
    fn lit_i(v: i64) -> ScalarExpr {
        ScalarExpr::Literal(Literal::Int(v))
    }
    fn lit_f(v: f64) -> ScalarExpr {
        ScalarExpr::Literal(Literal::Float(ordered_float::NotNan::new(v).unwrap()))
    }
    fn lit_s(s: &str) -> ScalarExpr {
        ScalarExpr::Literal(Literal::String(s.to_string()))
    }
    fn lit_b(b: bool) -> ScalarExpr {
        ScalarExpr::Literal(Literal::Bool(b))
    }
    ...
    fn infer_literals_types_and_nullability() {
        ...
        assert_eq!(
            TypeInference::infer_scalar(&lit_null(), &ctx).unwrap(),
            (JsonPrimitive::Null, true)
        );
        assert_eq!(
            TypeInference::infer_scalar(&lit_b(true), &ctx).unwrap(),
            (JsonPrimitive::Bool, false)
        );
        assert_eq!(
            TypeInference::infer_scalar(&lit_i(1), &ctx).unwrap(),
            (JsonPrimitive::Int, false)
        );
        assert_eq!(
            TypeInference::infer_scalar(&lit_f(1.0), &ctx).unwrap(),
            (JsonPrimitive::Float, false)
        );
        assert_eq!(
            TypeInference::infer_scalar(&lit_s("x"), &ctx).unwrap(),
            (JsonPrimitive::String, false)
        );
    }
    ...
    fn infer_column_uses_schema_and_qualification() {
        ...
        assert_eq!(
            TypeInference::infer_scalar(&col_unq("a"), &ctx).unwrap(),
            (JsonPrimitive::Int, false)
        );
        assert_eq!(
            TypeInference::infer_scalar(&col_unq("s"), &ctx).unwrap(),
            (JsonPrimitive::String, true)
        );

        // qualified works too
        assert_eq!(
            TypeInference::infer_scalar(&col("t", "s"), &ctx).unwrap(),
            (JsonPrimitive::String, true)
        );
    }
    ...
    fn infer_on_wildcards_errors() {
        ...
        let err2 =
            TypeInference::infer_scalar(&ScalarExpr::WildCardWithCollection("t".into()), &ctx);
        assert!(err1.is_err());
        assert!(err2.is_err());
    ...
    fn infer_scalar_functions_simple() {
        ...
        let (ty, nullable) =
            TypeInference::infer_scalar(&fun("upper", vec![col_unq("s")]), &ctx).unwrap();
        assert_eq!(ty, JsonPrimitive::String);
        assert!(nullable);

        // length(s) -> Int, nullability follows arg
        let (ty2, nullable2) =
            TypeInference::infer_scalar(&fun("length", vec![col_unq("s")]), &ctx).unwrap();
    ...
    fn infer_coalesce_promotes_types_and_nullability_all_inputs_nullable() {
        ...
        let (ty, nullable) = TypeInference::infer_scalar(
            &fun("coalesce", vec![lit_null(), lit_i(1), lit_f(2.0)]),
            &ctx,
        )
        .unwrap();
    ...
    fn infer_coalesce_all_inputs_nullable_yields_nullable() {
        ...
        let (ty, nullable) =
            TypeInference::infer_scalar(&fun("coalesce", vec![lit_null(), lit_null()]), &ctx)
                .unwrap();
    ...
    fn infer_count_variants_are_int_and_not_nullable() {
        ...
        // COUNT(*)
        let (ty1, n1) =
            TypeInference::infer_scalar(&agg("count", vec![ScalarExpr::WildCard]), &ctx).unwrap();
        assert_eq!(ty1, JsonPrimitive::Int);
        assert!(!n1);

        // COUNT(a)
        let (ty2, n2) =
            TypeInference::infer_scalar(&agg("count", vec![col_unq("a")]), &ctx).unwrap();
        assert_eq!(ty2, JsonPrimitive::Int);
        assert!(!n2);
    ...
    fn infer_sum_int_is_int_sum_float_is_float_nullable_true() {
        ...
        let (ty_i, n_i) =
            TypeInference::infer_scalar(&agg("sum", vec![col_unq("i")]), &ctx).unwrap();
        let (ty_f, n_f) =
            TypeInference::infer_scalar(&agg("sum", vec![col_unq("f")]), &ctx).unwrap();
    ...
    fn infer_avg_returns_float_nullable_true_for_numeric_inputs() {
        ...
        let (ty_i, n_i) =
            TypeInference::infer_scalar(&agg("avg", vec![col_unq("i")]), &ctx).unwrap();
        let (ty_f, n_f) =
            TypeInference::infer_scalar(&agg("avg", vec![col_unq("f")]), &ctx).unwrap();
    ...
    fn infer_min_max_return_same_type_nullable_true() {
        ...
        let (ty_min, n_min) =
            TypeInference::infer_scalar(&agg("min", vec![col_unq("s")]), &ctx).unwrap();
        let (ty_max, n_max) =
            TypeInference::infer_scalar(&agg("max", vec![col_unq("i")]), &ctx).unwrap();
    ...
    fn infer_sum_with_non_numeric_errors() {
        ...
        match err {
            Err(AnalyzerError::FunctionArgMismatch { name, .. }) => {
                assert_eq!(name.to_ascii_lowercase(), "sum")
            }
    ...
    fn infer_avg_with_non_numeric_errors() {
        ...
        assert!(
            matches!(err, Err(AnalyzerError::FunctionArgMismatch{ name, .. }) if name.eq_ignore_ascii_case("avg"))
        );
    }
    ...
    fn infer_min_max_without_args_errors() {
        ...
        assert!(
            matches!(err_min, Err(AnalyzerError::FunctionArgMismatch{ name, .. }) if name.eq_ignore_ascii_case("min"))
        );
        assert!(
            matches!(err_max, Err(AnalyzerError::FunctionArgMismatch{ name, .. }) if name.eq_ignore_ascii_case("max"))
        );
    }

    struct SP {
        by: std::collections::HashMap<String, SchemaDict>,
    }
    impl SP {
        fn new() -> Self {
            Self {
        ...
        fn with(mut self, name: &str, fields: Vec<(&str, JsonPrimitive, bool)>) -> Self {
            let mut m = IndexMap::new();
            for (k, t, n) in fields {
                m.insert(k.to_string(), FieldInfo { ty: t, nullable: n });
            }
    ...
    impl SchemaProvider for SP {
        fn schema_of(&self, s: &str) -> Option<SchemaDict> {
            self.by.get(s).cloned()
        }
    }
