...
impl WildcardResolver {
    pub fn expand_wildcard(
        expr: &ScalarExpr,
        ctx: &AnalysisContext,
    ) -> Result<Vec<ScalarExpr>, AnalyzerError> {
        match expr {
            ScalarExpr::WildCard => {
                ...
                for (visible_collection, backing) in &ctx.collections {
                    if let Some(schema) = ctx.schema_of_collection_ref(backing) {
                        for (col, _fi) in schema.fields {
                            result.push(ScalarExpr::Column(Column::WithCollection {
                                collection: visible_collection.clone(),
                                name: col,
                            }));
                        }
                    } else {
            ...
            ScalarExpr::WildCardWithCollection(collection) => {
                ...
                let schema = ctx
                    .schema_of_collection_ref(backing)
...
mod tests {
    ...
    fn star_expands_all_visible_collections_and_all_fields() {
        ...
        let out = WildcardResolver::expand_wildcard(&ScalarExpr::WildCard, &ctx).expect("expand *");
        ...
        assert_eq!(
            ...
            "field order for t1 preserved"
        );
        assert_eq!(
            t2_cols,
            vec!["x".to_string()],
            "field order for t2 preserved"
        );
    }
    ...
    fn table_star_expands_only_that_collection() {
        let sp = DummySchemas::new()
            ...
            .with("orders", vec![("total", JsonPrimitive::Float, false)]);
        // visible aliases u -> users, o -> orders
        let ctx = ctx_with(&sp, &[("users", Some("u")), ("orders", Some("o"))]);

        let out = WildcardResolver::expand_wildcard(
            &ScalarExpr::WildCardWithCollection("u".into()),
            &ctx,
        )
        .expect("expand u.*");

        let cols: Vec<(String, String)> = out
            .into_iter()
            .map(|e| match e {
                ScalarExpr::Column(Column::WithCollection { collection, name }) => {
                    (collection, name)
        ...
        assert_eq!(
            cols,
            vec![("u".into(), "id".into()), ("u".into(), "name".into()),]
        );
    }
    ...
    fn table_star_unknown_visible_collection_errors() {
        ...
        let err = WildcardResolver::expand_wildcard(
            &ScalarExpr::WildCardWithCollection("v".into()),
            &ctx,
        );
        match err {
    ...
    fn table_star_on_empty_schema_returns_empty_vec() {
        ...
        let out = WildcardResolver::expand_wildcard(
            &ScalarExpr::WildCardWithCollection("empty".into()),
            &ctx,
        )
        .expect("expand empty.*");
        assert!(
