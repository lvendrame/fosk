use crate::parser::{
    analyzer::{
        AggregateResolver, AnalysisContext, AnalyzedIdentifier, AnalyzerError, ColumnKey,
        ColumnResolver, ScalarResolver,
    },
    ast::{Column, Literal, OrderBy, ScalarExpr},
};
    ...
    fn rewrite_ob_expr_non_agg(
        ...
        match e {
            ...
            ScalarExpr::Column(Column::Name { name }) => {
                ...
                let (qc, _) =
                    ColumnResolver::qualify_column(&Column::Name { name: name.clone() }, ctx)?;
...
mod tests {
    ...
    fn order_by_positional_oob_errors() {
        ...
        let err0 = OrderByResolver::qualify_order_by(
            &[OrderBy {
                expr: ScalarExpr::Literal(Literal::Int(0)),
                ascending: true,
            }],
            &projection,
            &mut ctx,
    ...
    fn order_by_rejects_non_grouped_column_outside_aggregates_in_agg_query() {
        ...
        assert!(
            err.is_err(),
            "should reject non-grouped column in ORDER BY for agg query"
        );
        let msg = format!("{err:?}").to_lowercase();
        assert!(
            msg.contains("order by"),
            "error message should mention ORDER BY; got {msg}"
        );
    }
    ...
    fn order_by_allows_aggregate_expressions() {
        ...
        assert!(
            matches!(out[0].expr, ScalarExpr::Function(Function { ref name, .. }) if name.eq_ignore_ascii_case("count"))
        );
    }
    ...
    fn order_by_positional_negative_is_out_of_range() {
        ...
        let order = vec![OrderBy {
            expr: ScalarExpr::Literal(Literal::Int(-1)),
            ascending: true,
        }];
        let err = OrderByResolver::qualify_order_by(&order, &projection, &mut ctx, &group_set);
        assert!(err.is_err());
        let msg = format!("{err:?}").to_lowercase();
        assert!(
            msg.contains("order by position") || msg.contains("out of range"),
            "unexpected error: {msg}"
        );
    }
    ...
    fn order_by_scalar_over_non_grouped_column_is_rejected_in_agg_query() {
        // schema: t(age int)
        let sp = DummySchemas::new().with("t", vec![("age", JsonPrimitive::Int, false)]);
        let mut ctx = build_ctx_with_table(&sp, "t", None);
        ...
        let msg = format!("{err:?}").to_lowercase();
        assert!(
            msg.contains("order by") && msg.contains("group by"),
            "unexpected error: {msg}"
        );
    }
