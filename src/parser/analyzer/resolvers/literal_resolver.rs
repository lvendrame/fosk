    ...
    pub fn literal_equal(a: &Literal, b: &Literal) -> bool {
        match (a, b) {
            ...
            (Literal::Float(x), Literal::Float(y)) => {
                Self::float_eq(x.into_inner(), y.into_inner())
            }
...
mod tests {
    ...
    fn lf(v: f64) -> Literal {
        Literal::Float(ordered_float::NotNan::new(v).unwrap())
    }
    ...
    fn literal_equal_basic_and_cross_numeric() {
        ...
        // strings
        assert!(LiteralResolver::literal_equal(
            &String("abc".into()),
            &String("abc".into())
        ));
        assert!(!LiteralResolver::literal_equal(
    ...
    fn compare3_numeric_variants_and_edges() {
        ...
        assert_eq!(
            LiteralResolver::eval_compare3(&lf(1.0), Eq, &lf(1.0 + 1e-12)),
            True
        );
        ...
        assert_eq!(
            LiteralResolver::eval_compare3(&Int(2), Lt, &lf(2.000001)),
            True
        );
        ...
        assert_eq!(
            LiteralResolver::eval_compare3(&String("a".into()), Eq, &String("a".into())),
            True
        );
        assert_eq!(
            LiteralResolver::eval_compare3(&String("a".into()), NotEq, &String("b".into())),
            True
        );
        assert_eq!(
            LiteralResolver::eval_compare3(&String("a".into()), Lt, &String("b".into())),
            Unknown
        );
    }
    ...
    fn compare3_booleans_and_null_semantics() {
        ...
        assert_eq!(
            LiteralResolver::eval_compare3(&Bool(true), Eq, &Bool(true)),
            True
        );
        assert_eq!(
            LiteralResolver::eval_compare3(&Bool(true), NotEq, &Bool(false)),
            True
        );
        assert_eq!(
            LiteralResolver::eval_compare3(&Bool(true), Lt, &Bool(false)),
            Unknown
        );
    ...
    fn compare3_le_ge_with_tiny_delta() {
        ...
        assert_eq!(
            LiteralResolver::eval_compare3(&lf(1.0), LtEq, &lf(1.0 + 1e-12)),
            True
        );
        assert_eq!(
            LiteralResolver::eval_compare3(&lf(1.0 + 1e-12), GtEq, &lf(1.0)),
            True
        );
    }
