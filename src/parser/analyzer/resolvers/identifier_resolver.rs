use crate::parser::{
    analyzer::{AnalysisContext, AnalyzerError, WildcardResolver},
    ast::Identifier,
};
...
impl IdentifierResolver {
    pub fn expand_projection_idents(
        proj: &[Identifier],
        ctx: &AnalysisContext,
    ) -> Result<Vec<Identifier>, AnalyzerError> {
        let mut result = Vec::new();
        for id in proj {
            let expr = WildcardResolver::expand_wildcard(&id.expression, ctx)?;
            if expr.len() == 1 {
                result.push(Identifier {
                    expression: expr.into_iter().next().unwrap(),
...
mod tests {
    ...
    fn no_wildcard_preserves_alias() {
        ...
        let proj = vec![ident(
            ScalarExpr::Column(Column::Name { name: "a".into() }),
            Some("aa"),
        )];
        ...
        assert!(
            matches!(out[0].expression, ScalarExpr::Column(Column::Name { ref name }) if name == "a")
        );
    }
    ...
    fn star_expands_all_visible_collections_in_order_and_drops_aliases() {
        ...
        let cols: Vec<(String, String, Option<String>)> = out
            .into_iter()
            .map(|id| {
                let alias = id.alias;
                match id.expression {
                    ...
                    other => {
                        panic!("expected qualified column after wildcard expand, got {other:?}")
                    }
                }
            })
            .collect();
    ...
    fn table_star_expands_only_that_collection_and_keeps_alias_when_single_column() {
        ...
        let proj = vec![ident(
            ScalarExpr::WildCardWithCollection("t1".into()),
            Some("alias"),
        )];
        let out = IdentifierResolver::expand_projection_idents(&proj, &ctx).expect("expand");
        assert_eq!(out.len(), 1);
        ...
        assert_eq!(
            names,
            vec![("t2".into(), "a".into()), ("t2".into(), "b".into())]
        );
    }
    ...
    fn mixed_projection_expands_in_place_order() {
        ...
        // 4) trailing 'b'
        assert!(
            matches!(out[3].expression, ScalarExpr::Column(Column::Name{ ref name }) if name=="b")
        );
    }
