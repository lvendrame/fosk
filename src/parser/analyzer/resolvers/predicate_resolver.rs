use crate::parser::{
    analyzer::{AnalysisContext, AnalyzerError, LiteralResolver, ScalarResolver},
    ast::{Literal, Predicate, ScalarExpr, Truth},
};
...
impl PredicateResolver {
    pub fn fold_predicate(pred: &Predicate) -> Predicate {
        match pred {
                ...
                for p in list {
                    match Self::fold_predicate(p) {
                        Predicate::Const3(t) => {
                            acc = acc.and(t);
                            if acc == Truth::False {
                                return Predicate::Const3(Truth::False);
                            }
                        }
                ...
                for p in list {
                    match Self::fold_predicate(p) {
                        Predicate::Const3(t) => {
                            acc = acc.or(t);
                            if acc == Truth::True {
                                return Predicate::Const3(Truth::True);
                            }
                        }
                        other => out.push(other),
                    }
            ...
            Predicate::Compare { left, op, right } => {
                ...
                if let (Some(ll), Some(rr)) = (
                    ScalarResolver::scalar_literal(&l),
                    ScalarResolver::scalar_literal(&r),
                ) {
            ...
            Predicate::InList {
                ...
                let lits: Option<Vec<Literal>> = if list_folded
                    .iter()
                    .all(|x| ScalarResolver::scalar_literal(x).is_some())
                {
                    Some(
                        list_folded
                            .iter()
                            .map(|x| ScalarResolver::scalar_literal(x).unwrap())
                            .collect(),
                    )
                } else {
                    None
                ...
                if let (Some(elit), Some(set)) = (el, lits) {
                    ...
                    for v in &set {
                        if matches!(v, Literal::Null) {
                            has_null = true;
                        } else if LiteralResolver::literal_equal(&elit, v) {
                            found = true;
                            break;
                        }
                    }
            ...
            Predicate::Like {
                ...
                match (
                    ScalarResolver::scalar_literal(&e),
                    ScalarResolver::scalar_literal(&p),
                ) {
                    ...
                    (Some(Literal::Null), _) | (_, Some(Literal::Null)) => {
    ...
    pub fn qualify_predicate(
        predicate: &Predicate,
        ctx: &mut AnalysisContext,
    ) -> Result<Predicate, AnalyzerError> {
        Ok(match predicate {
            Predicate::And(v) => Predicate::And(
                v.iter()
                    .map(|x| Self::qualify_predicate(x, ctx))
            ...
            Predicate::Compare { left, op, right } => Predicate::Compare {
                left: ScalarResolver::qualify_scalar(left, ctx, false)?,
                op: *op,
                right: ScalarResolver::qualify_scalar(right, ctx, false)?,
            },
            Predicate::IsNull { expr, negated } => Predicate::IsNull {
                expr: ScalarResolver::qualify_scalar(expr, ctx, false)?,
                negated: *negated,
            },
            Predicate::InList {
                expr,
                ...
                list: list
                    .iter()
                    .map(|x| ScalarResolver::qualify_scalar(x, ctx, true))
                    .collect::<Result<Vec<_>, _>>()?,
                negated: *negated,
            },
            Predicate::Like {
                ...
                expr: ScalarResolver::qualify_scalar(expr, ctx, false)?,
                pattern: ScalarResolver::qualify_scalar(pattern, ctx, false)?,
                negated: *negated,
            },
...
mod tests {
    ...
    use crate::{
        ...
        parser::{
            analyzer::PredicateResolver,
            ast::{Column, ComparatorOp, Function, Predicate, ScalarExpr, Truth},
        },
    };
    ...
    fn fold_is_null_and_not_null() {
        ...
        assert_eq!(
            PredicateResolver::fold_predicate(&p),
            Predicate::Const3(Truth::True)
        );
        // IS NOT NULL
        let p = Predicate::IsNull {
            expr: ScalarExpr::Literal(Literal::String("x".into())),
            negated: true,
        };
        assert_eq!(
            PredicateResolver::fold_predicate(&p),
    ...
    fn fold_in_and_not_in() {
        ...
        let p = Predicate::InList {
            expr: ScalarExpr::Literal(Literal::Int(5)),
            list: vec![
                ScalarExpr::Literal(Literal::Int(1)),
                ScalarExpr::Literal(Literal::Int(5)),
                ScalarExpr::Literal(Literal::Int(7)),
            ],
            negated: false,
        };
        assert_eq!(
    ...
    fn lit_s(v: &str) -> ScalarExpr {
        ScalarExpr::Literal(Literal::String(v.to_string()))
    }
    fn lit_b(v: bool) -> ScalarExpr {
    ...
    fn like_underscore_is_exactly_one_char() {
        ...
        let ok = Predicate::Like {
            expr: lit_s("abd"),
            pattern: lit_s("a_d"),
            negated: false,
        };
        let no0 = Predicate::Like {
            expr: lit_s("ad"),
            pattern: lit_s("a_d"),
            negated: false,
        };
        let no2 = Predicate::Like {
            expr: lit_s("abdd"),
            pattern: lit_s("a_d"),
            negated: false,
        };
        assert!(matches!(
            PredicateResolver::fold_predicate(&ok),
            Predicate::Const3(Truth::True)
        ));
        assert!(matches!(
            PredicateResolver::fold_predicate(&no0),
            Predicate::Const3(Truth::False)
        ));
        assert!(matches!(
            PredicateResolver::fold_predicate(&no2),
            Predicate::Const3(Truth::False)
        ));
    }
    ...
    fn like_is_case_insensitive() {
        let p = Predicate::Like {
            expr: lit_s("HelloWorld"),
            pattern: lit_s("he%world"),
            negated: false,
        };
        assert!(matches!(
            PredicateResolver::fold_predicate(&p),
    ...
    fn like_escape_percent_and_underscore() {
        ...
        let p1 = Predicate::Like {
            expr: lit_s("he%llo"),
            pattern: lit_s(r"he\%l%"),
            negated: false,
        };
        assert!(matches!(
            PredicateResolver::fold_predicate(&p1),
            Predicate::Const3(Truth::True)
        ));
        ...
        let p2 = Predicate::Like {
            expr: lit_s("a_c"),
            pattern: lit_s(r"a\_c"),
            negated: false,
        };
        assert!(matches!(
            PredicateResolver::fold_predicate(&p2),
            Predicate::Const3(Truth::True)
        ));
    }
    ...
    fn qualify_predicate_qualifies_columns_and_nested_structures() {
        ...
        let ok = match q {
            Predicate::And(v) => {
                ...
                let lq_ok = match &v[0] {
                    Predicate::Compare {
                        left: ScalarExpr::Function(Function { args, .. }),
                        ..
                    } => match &args[0] {
                ...
                let rq_ok = match &v[1] {
                    Predicate::InList {
                        expr: ScalarExpr::Column(Column::WithCollection { collection, name }),
                        ..
                    } => collection == "t" && name == "a",
                    _ => false,
                };
    ...
    fn qualify_predicate_errors_on_wildcard_outside_count() {
        ...
        assert!(
            msg.contains("wildcards") || msg.contains("wildcard"),
    ...
    fn fold_compare_booleans_only_eq_noteq_defined() {
        use ComparatorOp::*;
        assert!(matches!(
            PredicateResolver::fold_predicate(&Predicate::Compare {
                left: lit_b(true),
                op: Eq,
                right: lit_b(true)
            }),
