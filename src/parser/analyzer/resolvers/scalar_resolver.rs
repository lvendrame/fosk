...
impl ScalarResolver {
        ...
        match expr {
            ScalarExpr::Function(Function {
                ...
                if matches!(lname.as_str(), "count" | "sum" | "avg" | "min" | "max") {
                    return ScalarExpr::Function(Function {
                        name: name.clone(),
                        args: args.clone(),
                        distinct: *distinct,
                    });
                ...
                for arg in &folded_args {
                    if let ScalarExpr::Literal(l) = arg {
                        lit_args.push(l.clone());
                    } else {
                        return ScalarExpr::Function(Function {
                            name: name.clone(),
                            args: folded_args,
                ...
                let folded = match (lname.as_str(), lit_args.as_slice()) {
                    ("upper", [Literal::String(value)]) => {
                        Some(Literal::String(value.to_uppercase()))
                    }
                    ("lower", [Literal::String(value)]) => {
                        Some(Literal::String(value.to_lowercase()))
                    }
                    ("trim", [Literal::String(value)]) => {
                        Some(Literal::String(value.trim().to_string()))
                    }
                    ("length", [Literal::String(value)]) => {
                        Some(Literal::Int(value.chars().count() as i64))
                    }
                    _ => None,
                };

                folded.map(ScalarExpr::Literal).unwrap_or_else(|| {
                    ScalarExpr::Function(Function {
                        name: name.clone(),
                        args: folded_args,
                        distinct: *distinct,
    ...
    pub fn qualify_scalar(
        expr: &ScalarExpr,
        ctx: &mut AnalysisContext,
        allow_args: bool,
    ) -> Result<ScalarExpr, AnalyzerError> {
        match expr {
            ScalarExpr::Column(c) => Ok(ScalarExpr::Column(
                ColumnResolver::qualify_column(c, ctx)?.0,
            )),
            ...
            ScalarExpr::Parameter => Self::qualify_parameter(ctx, allow_args),
            ...
            ScalarExpr::WildCard | ScalarExpr::WildCardWithCollection(_) => Err(
                AnalyzerError::Other("wildcards must be expanded before qualification".into()),
            ),
    ...
    fn expand_parameter_value(json_value: &Value, allow_args: bool) -> Option<ScalarExpr> {
        match json_value {
            ...
            Value::Number(number) => {
                if number.is_f64() {
                    Some(ScalarExpr::Literal(Literal::Float(
                        NotNan::new(number.as_f64().unwrap()).unwrap(),
                    )))
...
mod tests {
    ...
    fn scalar_literal_only_for_literal() {
        ...
        assert!(
            ScalarResolver::scalar_literal(&ScalarExpr::Column(Column::Name { name: "x".into() }))
                .is_none()
        );
    }
    ...
    fn fold_scalar_folds_simple_and_nested_scalar_functions() {
        ...
        assert_eq!(
            folded,
            ScalarExpr::Literal(Literal::String("hello".to_uppercase()))
        );
    }
    ...
    fn fold_scalar_does_not_fold_when_args_not_all_literals() {
        ...
        let expr = ScalarExpr::Function(Function {
            name: "length".into(),
            distinct: false,
            args: vec![ScalarExpr::Column(Column::Name {
                name: "name".into(),
            })],
        });
        let folded = ScalarResolver::fold_scalar(&expr);
        assert!(
            matches!(folded, ScalarExpr::Function(Function { name, .. }) if name.eq_ignore_ascii_case("length"))
        );
    }
    ...
    fn qualify_scalar_allows_count_star_and_keeps_wildcard() {
        let sp = DummySchemas::new().with("t", vec![("id", JsonPrimitive::Int, false)]);
        ...
        let qualified =
            ScalarResolver::qualify_scalar(&expr, &mut ctx, false).expect("qualify COUNT(*)");
    ...
    fn qualify_scalar_errors_on_wildcard_outside_count() {
        ...
        assert!(
            msg.contains("wildcards must be expanded"),
    ...
    fn qualify_scalar_qualifies_columns_inside_function_args() {
        // table t(name string)
        let sp = DummySchemas::new().with("t", vec![("name", JsonPrimitive::String, false)]);
        let mut ctx = ctx_for_single_table(&sp, "t", None);
    ...
    fn qualify_scalar_parameter_three() {
        ...
        match (qualified1, qualified2, qualified3) {
            ...
            other => panic!(
                "expected (Literal::Int(1), Literal::String('value'), Literal::Bool(true)), got {other:?}"
            ),
        }
    }
    ...
    fn qualify_scalar_parameter_args() {
        ...
        match (qualified1, qualified2) {
            (ScalarExpr::Literal(Literal::Int(v1)), ScalarExpr::Args(args)) => {
                assert_eq!(v1, 1);
                assert_eq!(args.len(), 3);
            }
            other => panic!("expected (Literal::Int(1), Args(Vec<ScalarExpr>)), got {other:?}"),
        }
    }
