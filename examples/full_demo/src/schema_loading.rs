...
use crate::helpers::{app_file, required, schema_summary, schema_summary_fields};
...
pub fn run() -> Result<(), Box<dyn Error>> {
    ...
    let users = required(db.get("users"), "users schema collection should exist")?;
    println!(
        "Users config inferred from schema: {:?}",
        users.get_config()?
    );
    let users_schema = required(users.schema()?, "users schema should be loaded")?;
    println!("Users schema: {}", schema_summary(&users_schema));
    ...
    let with_refs = required(
        db.schema_with_refs_of("orders"),
        "orders schema with refs should exist",
    )?;
    println!(
    ...
    let status = db.load_collection_schema_from_file(&profile_schema_os_path)?;
    println!("{status}");

    let db_schema_os_path = app_file("mocks/schemas/database_schema.json").into_os_string();
    let status = db.load_schemas_from_file(&db_schema_os_path)?;
    println!("{status}");
    ...
    for relative_path in alternate_schema_files {
        ...
        let status = catalog_db.load_collection_schema_from_file(&path)?;
        println!("  {status}");
    }
    ...
    let status = rest_schema_db.load_schemas_from_file(&rest_schema_os_path)?;
    println!("{status}");
    println!(
    ...
    assert_eq!(users_schema.fields["user_id"].ty, JsonPrimitive::Int);
    let user_ref = required(
        db.get_collection_column_ref("orders", "user_id"),
        "orders.user_id reference should be inferred",
    )?;
    assert_eq!(user_ref.ref_collection, "users");
